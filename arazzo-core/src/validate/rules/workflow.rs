...
use crate::validate::rules::{actions, common::{validate_map_keys, validate_runtime_expr}, parameters, step};
...
pub(crate) fn validate_workflow(v: &mut Validator, wf: &Workflow, path: &str) {
    ...
    if let Some(success_actions) = &wf.success_actions {
        actions::validate_success_action_list(v, &format!("{path}.successActions"), success_actions, None);
    }
    if let Some(failure_actions) = &wf.failure_actions {
        actions::validate_failure_action_list(v, &format!("{path}.failureActions"), failure_actions, None);
    }
    ...
    for (idx, s) in wf.steps.iter().enumerate() {
        ...
        if !ID_RE.is_match(&s.step_id) {
            v.push(format!("{spath}.stepId"), "must match regex [A-Za-z0-9_\\-]+");
        }
        if !step_ids.insert(s.step_id.clone()) {
            v.push(format!("{spath}.stepId"), "must be unique within the workflow");
        }
